...
# Docker Build and Publish Script for Objectified Browse
...
test_image() {
    ...
    local test_container="objectified-browse-test-$$"
    docker run -d \
        --name "$test_container" \
        -p 3001:3000 \
...
# Deployment script for Objectified Browse
...
echo "Deploying Objectified Browse..."
...
docker stop objectified-browse 2>/dev/null || true
docker rm objectified-browse 2>/dev/null || true
...
echo ""
echo "Container logs:"
docker logs --tail 50 objectified-browse
EOF
...
services:
  objectified-browse:
    ...
    container_name: objectified-browse
    restart: unless-stopped
    ports:
      - "3000:3000"
    environment:
...
create_deployment_package() {
    ...
    local package_dir="objectified-browse-deploy-${VERSION}"
...
# Objectified Browse Deployment Package
...
Check logs: \`docker logs objectified-browse\`
Stop: \`docker stop objectified-browse\`
Restart: \`docker restart objectified-browse\`
EOF
...
# Main execution
main() {
    print_info "=== Objectified Browse Docker Build Script ==="
    echo ""
    ...
    [ "$do_package" = true ] && echo "  4. Transfer: objectified-browse-deploy-${VERSION}.tar.gz to remote server"
    echo ""
}
