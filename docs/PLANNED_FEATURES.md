...
| 5 | ~~[#2703](https://github.com/KenSuenobu/objectified-commercial/issues/2703)~~ | ~~CR-05~~ **shipped** | **ADE → Versions:** **Timeline | Change report** tab; view/edit persisted report (**`DOMPurify`** + **Mustache** preview); **Templates** dialog (create, project/tenant defaults); **Regenerate**; **`NEXT_PUBLIC_CHANGE_REPORT_UI=0`** to hide |
| 6 | ~~[#2704](https://github.com/KenSuenobu/objectified-commercial/issues/2704)~~ | ~~CR-06~~ **shipped** | Golden `ChangeReportModel` + Mustache render snapshots; `POST …/publish` → `insert_change_report_if_absent` integration test; Playwright **`e2e/change-report.spec.ts`** (mocked REST, no Ollama) |
...
| C.9 | **Property assist:** suggestions, dropdown, bulk, triggers, analysis | **#269–#276**, **#277** (done), **#278** (done) |
...
1. Prompt → **preview** → refine in preview / accept / reject; schema matches **classes**/**versions** (**#528–#532**, incl. iterative refinement **#532**).
