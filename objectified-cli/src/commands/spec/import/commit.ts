...
import {
  formatShortJobId,
  pickImportJobProjectSlug,
  pickImportJobVersionLabel,
...
export default class SpecImportCommit extends BaseCommand {
  static description =
  ...
  static seeAlso = ["spec import status", "spec import cancel", "spec import", "docs errors"];
  ...
  async run(): Promise<void> {
    ...
    await ensureImportMutationConfirmed({ yes: this.flags.yes === true, verbLabel: "commit" });

    this.ensureAuthenticated();

    let job;
    try {
      job = await this.api.commitImportJob(tenant, jobId);
    } catch (e) {
      if (e instanceof ObjectifiedCliError && e.exitCode === EXIT_CODES.CONFLICT) {
        throw new ObjectifiedCliError({
          ...
          hint:
            "Commit requires state pending-approval; run `objectified spec import status <job-id>` to inspect.",
    ...
    const slug = pickImportJobProjectSlug(job) ?? "?";
    const vid = pickImportJobVersionLabel(job) ?? "?";
    const langAscii = localePrefersAsciiTable(process.env);
    const mark = langAscii ? "[ok]" : "✔";
    const shortId = formatShortJobId(job.jobId);
    this.output.text(`${mark} Committed job ${shortId}. Project ${slug} now has draft revision ${vid}.`);
  }
}
