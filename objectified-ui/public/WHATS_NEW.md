...
- **Developer Mode `obj://` VFS**: `createObjVfs` in **`objectified-ui/src/lib/obj-vfs.ts`** implements **`readFile` / `writeFile` / `stat` / `watch` / `list`**, ~**500 ms** debounced coalesced writes per URI, **`If-Match`** / **`ETag`** on version saves (REST **`PUT /v1/versions/{tenant}/{project}/{id}`** returns **412** when stale), **`ObjVfsConflictDialog`** (reload / overwrite / diff), and a first virtual file **`obj://project/{projectId}/version/{versionId}/version-notes.json`** mapped to **`/api/versions`** (#3345).
- **Developer Mode workspace chrome**: reusable **`DeveloperModeWorkspaceChrome`** (rail, **`DeveloperModeVirtualFileTree`** with Radix **`Collapsible`** + tree keyboard navigation, sortable tabs, breadcrumbs, status bar) and **`diagnostic-counts`** helpers; **Paths → Code** view uses the shell when Developer Mode is on, with parse-marker aggregates in the status bar (#3346).
