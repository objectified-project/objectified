...
## CLI
- **`objectified-cli`**: `objectified spec import status|commit|cancel` — **`status`** (`GET …/{job_id}`, **`--watch`** until terminal with import polling exit semantics, **`--last`** + **`~/.cache/objectified/last-import-job-<tenant>.txt`**), **`commit`** / **`cancel`** (`POST …/commit` | `…/cancel`, **`--yes`** required off-TTY or exit **6**, HTTP **409** on commit shows API detail); cache file is written after each successful import **POST** (#3311).
- **`objectified-cli`**: `objectified spec import` adds CI ergonomics — **`--dry-run`** (sets **`options.dryRun`**, **`--json`** emits summary only), **`--review`** (stdout job id at **`pending-approval`**, exit **6** if the job skips that gate), **`--report <path>`** (append NDJSON + final **`summary`** line with **`exitCode`**), **`--ndjson`** (stream **`event`** / **`progress`** then terminal **`result`**; conflicts with **`--json`** exit **6**), **`--commit-on-complete`** / **`--no-commit-on-complete`** (forwards **`options.commitOnComplete`**); Ctrl-C while polling exits **1**; prose at **`objectified docs spec-import`** (#3310).
- **`objectified-cli`**: `objectified spec import <file>` without **`--project`** derives name / slug / description / OpenAPI metadata from the spec (like Map & Import), optional **`--name`** / **`--slug`** / **`--description`** / **`--domain`** / **`--visibility`**, TTY confirmation or **`--yes`** / **`--quiet`** / non-TTY rules, **`POST /v1/projects/{tenant_slug}`** then the usual import; slug collision exits **6**; import failures hint at **`projects delete <slug> --force`** (#3309).
- **`objectified-cli`**: `objectified spec import <file> --project <slug>` reads OpenAPI / Swagger / Arazzo (JSON or YAML), auto-detects `sourceKind`, resolves the project, POSTs **`/v1/imports/{tenant_slug}`** with `existingProjectId`, polls **`GET …/{job_id}`** with stderr progress (spinner respects **`--json`** / **`--quiet`**), and prints a completion summary (`--yes` accepted as a CI no-op); **`ImportJobCreateRequest`** wired through **`src/lib/client.ts`** (#3308).
