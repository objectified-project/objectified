...
## CLI
- **`objectified-cli`** / **`objectified-rest`**: OpenAPI and generated SDK now include the tenant-scoped specification-import job contract (`POST /v1/tenants/{tenant_slug}/imports` JSON+base64, `POST …/imports/upload` multipart, poll/cancel/commit/rollback); REST handlers return **501** until the importer is implemented—the contract is for upcoming `objectified import spec` (#3330). The dashboard Import flow remains Next.js server actions (`import-helper`), not these URLs.
