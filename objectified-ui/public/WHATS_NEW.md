...
## Repositories
- Added REPO-11.1 **repository attention rollup** (backend): `odb.repository_attention` materializes per-repo “needs attention” reasons and a weighted score; the REST service recomputes on scan completion, import jobs, spec selection changes, auto-pause, and an hourly safety pass, and issues `pg_notify` on `dashboard.attention` when the row is stored in PostgreSQL (for upcoming REPO-11.2/11.3 widgets).
