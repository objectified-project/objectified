...
## CLI
- **`objectified-cli`**: `objectified spec import <file>` without **`--project`** derives name / slug / description / OpenAPI metadata from the spec (like Map & Import), optional **`--name`** / **`--slug`** / **`--description`** / **`--domain`** / **`--visibility`**, TTY confirmation or **`--yes`** / **`--quiet`** / non-TTY rules, **`POST /v1/projects/{tenant}`** then the usual import; slug collision exits **6**; import failures hint at **`projects delete <slug> --force`** (#3309).
