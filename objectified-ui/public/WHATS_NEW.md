...
## Repositories
- Added REPO-10.1 **Scanned repository report** at `/ade/dashboard/repositories/reports` with server-paginated `GET /v1/repositories/{tenant}/scan-reports` (pre-aggregated per-scan `totalsJson` from in-memory `repository_scan_report` rows), provider/status/search filters in the query string, default sort by attention then last scan, and row links that deep-link into the repository (interim until REPO-10.2).
