...
export async function GET(
  ...
  try {
    ...
    const url = `${REST_API_BASE_URL}/repositories/${encodeURIComponent(auth.tenantSlug)}/${encodeURIComponent(params.id)}/specs/${encodeURIComponent(params.fileId)}/detail`;
    ...
    if (!response.ok) {
      ...
      const errText = typeof data.detail === 'string' ? data.detail : 'Failed to load spec detail';
      return NextResponse.json(
