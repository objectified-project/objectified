...
export async function GET() {
  try {
    ...
    const url = `${REST_API_BASE_URL}/repositories/${encodeURIComponent(auth.tenantSlug)}/scan-reports/exports`;
    const response = await fetch(url, { method: 'GET', headers: createRestAuthHeaders(auth.sessionUser) });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
      return NextResponse.json(
        { success: false, error: typeof (data as { detail?: string }).detail === 'string' ? (data as { detail: string }).detail : 'Failed' },
