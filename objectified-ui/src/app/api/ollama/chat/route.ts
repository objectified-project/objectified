...
- When the property name alone implies a shape, use these defaults: \`email\` → string with \`format: "email"\`; \`createdAt\` → string with \`format: "date-time"\`; \`age\` → integer with \`minimum: 0\`; \`price\` → number with \`minimum: 0\`; \`isActive\` → boolean.
