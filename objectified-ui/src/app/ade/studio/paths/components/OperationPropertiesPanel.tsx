  ...
  const handleSaveOperation = async () => {
    ...
    try {
      const metadata: Record<string, unknown> = {
        ...
        // Explicitly set deprecated to false when unchecked to overwrite existing value
        deprecated: deprecated ? true : false,
        // x-private: hide this operation from Swagger when true
          ...
          <Box sx={{ flex: 1, overflow: 'auto', p: 2 }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box sx={{ mt: 1 }}>
                <FormControlLabel
                  ...
                  label={
                    <span className="text-xs text-gray-700 dark:text-gray-300 flex items-center gap-1">
                      <span className={deprecated ? 'text-amber-600 dark:text-amber-400' : ''}>
                        Mark as deprecated
                      </span>
                      {deprecated && (
                ...
                />
                {deprecated && (
                  <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5 ml-7">
                    Consumers should avoid using this operation
                  </p>
                )}
              </Box>

              {/* Private (x-private): hide from Swagger */}
              <Box sx={{ mt: 1 }}>
                <FormControlLabel
                  control={
                    <Checkbox
                  ...
                  label={
                    <span className="text-xs text-gray-700 dark:text-gray-300 flex items-center gap-1">
                      ...
                      <span className={xPrivate ? 'text-indigo-600 dark:text-indigo-400' : ''}>
                        Hide from Swagger (x-private)
                      </span>
                      {xPrivate && (
                        <span className="text-[10px] px-1.5 py-0.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 rounded">
                          Private
                        </span>
                      )}
                    </span>
                ...
                />
                {xPrivate && (
                  <p className="text-xs text-indigo-600 dark:text-indigo-400 mt-0.5 ml-7">
                    This operation will not appear in the public Swagger/OpenAPI docs
                  </p>
                )}
              </Box>
