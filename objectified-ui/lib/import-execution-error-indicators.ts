...
/** Tailwind classes for Live Progress row by level (error = red, warn = amber, info = gray). */
export function getLiveProgressRowClasses(level: LogLevel): string {
  const base = 'flex items-start gap-2 p-2 rounded border ';
  if (level === 'error') return base + 'border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-950/30';
  if (level === 'warn') return base + 'border-yellow-200 dark:border-yellow-800 bg-amber-50/50 dark:bg-amber-950/20';
  return base + 'border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40';
}

/** Tailwind classes for Import Log line by level (error = red highlight and left border). */
export function getImportLogLineClasses(level: LogLevel): string {
  const base = 'text-xs font-mono rounded px-2 py-1 -mx-2 ';
  if (level === 'error') return base + 'bg-red-100 dark:bg-red-950/50 border-l-2 border-red-500 dark:border-red-500';
  if (level === 'warn') return base + 'bg-amber-50 dark:bg-amber-950/30';
  return base.trim();
}
